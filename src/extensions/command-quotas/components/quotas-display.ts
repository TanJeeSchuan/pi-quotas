...
export class QuotasComponent implements Component {
	...
	render(width: number): string[] {
		...
		if (this.state.type === "loading") {
			lines.push(
				...(this.loader
					...
					: [this.theme.fg("muted", "  Fetching quotas...")]),
			);
		} else {
		...
		lines.push(
			this.theme.fg(
				"dim",
				`  pi-quotas v${pkg.version}  ·  r to refresh  q/Esc to close`,
			),
		);
		lines.push(...border.render(width));
		return lines;
	...
	private renderProvider(snapshot: Snapshot, maxWidth: number): string[] {
		...
		lines.push(
			truncateToWidth(`  ${this.theme.fg("accent", title)}`, maxWidth),
		);

		if (!snapshot.result.success) {
			lines.push(
				truncateToWidth(
					`  ${this.theme.fg("warning", snapshot.result.error.message)}`,
					maxWidth,
				),
			);
			return lines;
		...
		if (windows.length === 0) {
			lines.push(
				truncateToWidth(
					`  ${this.theme.fg("dim", "No quota windows available")}`,
					maxWidth,
				),
			);
			return lines;
		...
		for (const window of windows) {
			...
			if (window.isCurrency) {
				...
				if (window.limitValue === 0) {
					const label = window.isRemaining ? "remaining" : "used";
					usedStr = `$${window.usedValue.toFixed(2)} ${label}`;
				} else {
				...
				const remaining = Math.max(
					0,
					Math.round(window.limitValue - window.usedValue),
				);
			...
			const limitedBadge = window.limited
				? this.theme.fg("error", " LIMITED")
				: "";
			...
			lines.push(
				truncateToWidth(
					`    ${this.theme.fg(labelColor, `${window.label}:`)}`,
					maxWidth,
				),
			);
			lines.push(
				truncateToWidth(
					`    ${bar} ${this.theme.fg(color, usedStr)}${limitedBadge}`,
					maxWidth,
				),
			);
			...
			if (window.resetsAt.getTime() > 0) {
				subtitleParts.push(
					`${window.nextLabel ?? "Resets"} in ${formatTimeRemaining(window.resetsAt)}`,
				);
