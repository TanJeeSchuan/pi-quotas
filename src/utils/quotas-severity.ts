...
export function assessWindow(window: QuotaWindow): RiskAssessment {
	...
	if (progress === null) {
		let severity = absoluteUsageSeverity(window, projectedPercent);

		// For "remaining balance" windows (e.g., DeepSeek), trigger warnings
		// based on absolute remaining amount rather than usedPercent.
		if (window.isRemaining) {
			if (window.usedValue <= 0) {
				severity = maxSeverity(severity, "critical");
			} else if (window.usedValue < 1) {
				severity = maxSeverity(severity, "high");
			} else if (window.usedValue < 5) {
